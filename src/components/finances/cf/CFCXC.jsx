...
/**
 * CFCXC — Cuentas por Cobrar integradas con Ventas.
 * Props: { transactions, db, appId, showToast, onRegisterAbono }
 */
export default function CFCXC({ transactions = [], db, appId, showToast, onRegisterAbono }) {
  const [search, setSearch] = useState('');
  const [filterStatus, setFilterStatus] = useState('pendiente');

  // CXC = ingresos con facturas/notas débito que no estén anuladas
  const cxcAll = useMemo(() =>
    transactions.filter(t =>
      t.type === 'ingreso' &&
      t.sriStatus !== 'anulado' &&
      ['factura', 'nota_debito', 'nota_venta'].includes(t.documentType)
    ),
    [transactions]
  );

  const filtered = useMemo(() => {
    let r = filterStatus !== 'all' ? cxcAll.filter(t => t.paymentStatus === filterStatus) : cxcAll;
    if (search) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200">
          ...
          <p className="text-xl font-bold text-emerald-900 mt-1">{formatMoney(totalPendiente)}</p>
          <p className="text-[10px] text-emerald-600 mt-1">{filtered.filter(t => t.paymentStatus !== 'pagado').length} documentos</p>
        </div>
        <div className="p-4 rounded-xl bg-rose-50 border border-rose-200">
          <p className="text-[10px] font-bold text-rose-700 uppercase">Vencido</p>
          <p className="text-xl font-bold text-rose-900 mt-1">{formatMoney(totalVencido)}</p>
          <p className="text-[10px] text-rose-600 mt-1">{filtered.filter(t => t.dueDate && t.dueDate < today && t.paymentStatus !== 'pagado').length} documentos</p>
        </div>
      </div>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={9} className="py-10 text-center text-slate-400">
                  Sin cuentas por cobrar con los filtros actuales
                </td>
              </tr>
            ) : (
              filtered.map(t => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-center">
                      {saldo > 0 && onRegisterAbono && (
                        <button
                          onClick={() => onRegisterAbono(t)}
