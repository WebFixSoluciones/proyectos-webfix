    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <div
            className={`flex items-center gap-2 px-3.5 py-2.5 rounded-xl border w-full sm:w-80 transition-all ${isDarkMode ? "bg-black/20 border-white/10 focus-within:border-violet-500/50 focus-within:ring-1 focus-within:ring-violet-500/30" : "bg-white border-gray-300 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500/30"}`}
          >
            <Search
              size={14}
          ...
          <select
            ...
            className={`px-3 py-2.5 rounded-xl border text-xs outline-none cursor-pointer transition-all ${isDarkMode ? "bg-black/20 border-white/10 text-gray-300 hover:bg-black/35" : "bg-white border-gray-300 text-gray-700 font-semibold hover:bg-gray-50"}`}
          >
      ...
      {/* TABLA CATÁLOGO */}
      <div
        className={`rounded-2xl border overflow-hidden backdrop-blur-xl ${isDarkMode ? "border-white/5 bg-white/[0.01] shadow-lg" : "border-gray-200 bg-white shadow-sm"}`}
      >
        {loading ? (
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left text-xs whitespace-nowrap">
              ...
              <tbody
                className={`divide-y ${isDarkMode ? "divide-white/5" : "divide-gray-200/80"}`}
              >
                {filtered.map((p) => {
                  ...
                  return (
                    <tr
                      key={p.id}
                      className={`transition-colors ${isDarkMode ? "hover:bg-white/[0.02]" : "hover:bg-gray-50/50"}`}
                    >
                      <td
                        className={`px-6 py-4 font-mono text-[10px] font-bold ${isDarkMode ? "text-gray-300" : "text-black"}`}
                      >
                        {p.sku}
                      </td>
                      <td className="px-6 py-4">
                        <div
                          className={`font-bold text-xs ${isDarkMode ? "text-white" : "text-black"}`}
                        >
                          {p.name}
                        </div>
                        <div className="flex flex-wrap gap-1 mt-1 text-[8px] font-bold uppercase tracking-wider">
                          ...
                          {p.bodega && (
                            <span
                              className={`px-1.5 py-0.5 rounded ${isDarkMode ? "bg-blue-500/10 text-blue-400 border border-blue-500/10" : "bg-blue-50 text-blue-750 border border-blue-200"}`}
                            >
                              Bodega: {p.bodega}
                            </span>
                          )}
                        ...
                        {p.description && (
                          <p
                            className={`text-[9px] font-normal truncate mt-1 max-w-[220px] ${isDarkMode ? "text-gray-500" : "text-black"}`}
                            title={p.description}
                          >
                            {p.description}
                          </p>
                      ...
                      <td
                        className={`px-6 py-4 text-right font-bold ${isDarkMode ? "text-gray-300" : "text-black"}`}
                      >
                      ...
                      <td
                        className={`px-6 py-4 font-bold ${isDarkMode ? "text-gray-350" : "text-black"}`}
                      >
                        {p.ivaCategory}%
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            ...
                            className={`p-2 rounded-xl transition-colors ${isDarkMode ? "hover:bg-blue-500/15 text-blue-400 border border-transparent" : "hover:bg-blue-50 text-blue-650 border border-gray-200"}`}
                            title="Editar"
                          >
                            <Edit2 size={13} />
                          </button>
                          <button
                            ...
                            className={`p-2 rounded-xl transition-colors ${isDarkMode ? "hover:bg-red-500/15 text-red-400 border border-transparent" : "hover:bg-red-50 text-red-650 border border-gray-200"}`}
                            title="Eliminar"
                          >
                            <Trash2 size={13} />
                          </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-md animate-in fade-in duration-200">
          <div
            className={`w-full max-w-lg p-6 sm:p-8 rounded-[2rem] shadow-2xl transition-all duration-300 border ${isDarkMode ? "glass-panel-dark text-white" : "glass-panel-light text-gray-900"}`}
          >
