...
const COLUMN_COLORS = [
 { id:'gray', badge:'bg-gray-200/60 text-gray-700 dark:bg-white/10 dark:text-gray-300', bgDark:'bg-text-primary/40 border-white/10', bgLight:'bg-gray-50/50 border-gray-200/80', dot:'bg-gray-400' },
...
export default function ErpDashboard({ projectsList, allTasksGlobal, setActivePageId, setVentasInitialSubTab, db, appId }) {
 ...
 const cardClass =`p-5 rounded-card border transition-all duration-300 surface-card border-gray-300/80 hover:border-gray-400 text-gray-900`;

 const currentGlassPanel ='surface-card border border-gray-250';
 ...
 {certStatus !=='ok' && (
 <div className={`p-4 rounded-card border flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs animate-in fade-in duration-300 ${
 certStatus ==='expired' 
 ...
 <div className={`p-5 rounded-card border bg-gray-50/50 border-gray-200/80`}>
 ...
 <div className={`p-3 rounded-card shrink-0 bg-emerald-50 text-emerald-700`}>
 <TrendingUp size={22} />
 </div>
 <div className="min-w-0 flex-1">
 <span className={`text-xs font-bold uppercase tracking-wider block text-gray-600`}>Ventas (Mes)</span>
 ...
 <div className={`p-3 rounded-card shrink-0 bg-red-50 text-red-700`}>
 <TrendingDown size={22} />
 </div>
 <div className="min-w-0 flex-1">
 <span className={`text-xs font-bold uppercase tracking-wider block text-gray-600`}>Gastos (Mes)</span>
 ...
 <div className={`p-3 rounded-card shrink-0 bg-primary-light text-primary`}>
 <Package size={22} />
 </div>
 <div className="min-w-0 flex-1">
 ...
 <div className="flex items-center gap-4 pt-4 sm:pt-0 lg:pl-6 sm:px-2">
 <div className={`p-3 rounded-card shrink-0 bg-purple-50 text-purple-700`}>
 <FolderOpen size={22} />
 </div>
 <div className="min-w-0 flex-1">
 ...
 <div className={`${currentGlassPanel} xl:col-span-7 p-4 sm:p-6 rounded-card`}>
 <div className="flex items-center justify-between mb-5">
 <h3 className={`text-base font-bold flex items-center gap-2 text-gray-800`}>
 <Briefcase size={18} className="text-primary" />
 Progreso de Proyectos Activos
 </h3>
 <span className={`text-xs font-bold px-2 py-0.5 rounded-full bg-gray-200 text-gray-850`}>
 ...
 <div className={`h-2 w-full rounded-full overflow-hidden border bg-gray-200 border-white/50`}>
 <div 
 ...
 <div className="flex justify-between items-center mt-2.5 text-xs font-medium text-gray-500">
 <span className="flex items-center gap-1">
 <CheckCircle2 size={11} className="text-emerald-500" />
 {doneTasksCount} hechas
 </span>
 <span className="flex items-center gap-1">
 <ListTodo size={11} className="text-primary" />
 ...
 <h4 className={`text-xs font-bold uppercase tracking-wider mb-2.5 flex items-center gap-1.5 text-gray-700`}>
 <BarChart3 size={12} />
 Estado de Tareas en toda la Empresa
 </h4>
 
 <div className={`h-3 w-full rounded-full overflow-hidden flex border border-gray-250 bg-gray-150`}>
 ...
 <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1.5 mt-3 text-xs font-semibold">
 {Array.from(new Set(projectsList.flatMap(p => (p.columns || DEFAULT_COLUMNS).map(c => c.id)))).map((statusId) => {
 const count = allTasksGlobal.filter(t => t.status === statusId).length;
 const colDef = projectsList.flatMap(p => (p.columns || DEFAULT_COLUMNS)).find(c => c.id === statusId) || { title: statusId, color:'gray' };
 ...
 <div className={`${currentGlassPanel} xl:col-span-5 p-4 sm:p-6 rounded-card flex flex-col justify-between`}>
 <div>
 <div className="flex items-center justify-between mb-5">
 ...
 <div className={`mt-5 pt-4 border-t text-xs flex justify-between items-center border-gray-250 text-gray-600`}>
 ...
 <div className={`p-4 sm:p-6 rounded-card relative overflow-hidden border bg-gradient-to-br from-[#f6f2fc] via-white to-white border-purple-200`}>
 ...
 {isGeneratingReport ? (
 <div className={`flex flex-col items-center justify-center py-12 rounded-card border border-dashed bg-gray-50/50 border-gray-250 shimmer-loading-light`}>
 ...
 <div className={`p-5 rounded-card border transition-all bg-white border-gray-250`}>
 <div className="prose prose-sm max-w-none prose-invert">
 {renderMarkdown(aiReport)}
 </div>
 </div>
 ) : (
 <div className={`p-6 rounded-card border text-center border-dashed bg-gray-50/50 border-gray-250`}>
